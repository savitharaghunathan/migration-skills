...
|---|---|---|---|---|---|
| Health probes enabled by default, no longer need explicit opt-in | behavioral | `management.endpoint.health.probes.enabled=true` | (remove the property — it's the default now) | Only affects apps that explicitly set this property | Health Probes |
| Thread pool executor replaced with virtual threads by default | behavioral | `@Bean TaskExecutor taskExecutor() { return new ThreadPoolTaskExecutor(); }` | (remove — virtual threads used automatically) | Add `spring.threads.virtual.enabled=false` to opt out | Virtual Threads |
